...
- **リンクテキストにファイル名をそのまま貼らない**：`2024-06-27_Foo_Bar_Baz.md` のようなファイル名をリンクテキストへ入れると、アンダースコアがMarkdownの強調として解釈され、`oxfmt` が斜体や太字へ変換して表示が崩れる。リンクテキストではアンダースコアをスペースに置き換える。
...
- `scripts/check_wiki_links.py` を実行し、`wiki/index.md` への掲載漏れ、リンク切れ、強調に化けたリンクテキストがないことを確認する。整形で崩れることがあるため、`oxfmt` の後に実行する。
- `git diff` で `sources/` に意図しない変更がないことを確認する。

`wiki/` に原資料の一覧ページ (参照マップ) を置いて全メモを網羅する運用にしている場合は、`scripts/check_wiki_links.py --check-unreferenced` を使うと、どのWikiページからも参照されていない原資料を検出できる。
その運用をしていない場合、Wiki化を見送ったメモがすべて報告されるため、このオプションは使わない。
