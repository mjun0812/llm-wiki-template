...
- **本文編集禁止**：frontmatterだけが編集対象。本文 (frontmatter以下) は1バイトも触らない。タイポも画像の相対パスも書き換えない。ただし `oxfmt` による整形 (表の桁揃え、記号のエスケープなど) はこの不変条件の例外とする。pre-commit hookが全Markdownに走るため、コミット時には必ず適用される。整形差分が出てもそのままコミットしてよい。
...
- カテゴリは出所ではなく内容 (トピック) で決める。AI回答による下調べや未検証の調査メモも、明確なトピックがあれば該当カテゴリに入れる。AI回答であることは frontmatter の `model` で区別でき、未検証の調査メモとして扱う点は変わらない。
...
- **アイデアとAI下調べが混ざっている**：内容のトピックで判定し、1ファイルのまま移す。トピックが定まらなければ `sources/ideas/` に入れる。本文に未検証である旨を追記しない。
