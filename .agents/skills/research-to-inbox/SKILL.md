...
- 本文の理解に有用な図 (アーキテクチャ図、ベンチマークグラフなど) を見つけたら、`curl` などで `inbox/images/` にダウンロードする。外部URLのまま本文に埋め込まない。ファイル名はスペースを含まない半角英数字にする。
...
- 図解が有効な論点 (構成、比較、推移など) があれば、対応する画像を取得している。図が不要なテーマなら省略してよい。
...
- 引用番号 `[n]` は参照一覧と対応させる。
- ダウンロードした画像は `images/<ファイル名>` の相対パスで本文に埋め込み、その画像の出典にも `[n]` を付ける。
...
- 画像を保存した場合は `scripts/check_image_links.py inbox/` を実行し、画像リンクが壊れていないことを確認する。
